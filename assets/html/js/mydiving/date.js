(function ($) {
    $.fn.date = function (options,Ycallback,Ncallback) {
        ...
        var initYY=nowdate.getFullYear();
        ...
        if(initM==2){
            if(!isNaN(parseInt(pYear))){
                if((initY%4==0 && initY%100!=0)||(initY%100==0 && initY%400==0)){
                    enddays=29;
                }else{
                    enddays=28;
                }
            }
        }else if(initM==1 || initM==3 || initM==5 || initM==7 || initM==8 || initM==10 || initM==12){
            enddays=31;
        }else{
            enddays=30;
        }
        ...
        $.fn.date.defaultOptions = {
            beginyear:initY,                 //日期--年--份开始
            endyear:25,                   //日期--年--份结束
            beginmonth:initM,                   //日期--月--份结束
            endmonth:12,                    //日期--月--份结束
            beginday:initD,                     //日期--日--份结束
            endday:enddays,                      //日期--日--份结束
            beginhour:initH,
            endhour:24,
            beginminute:initI,
            endminute:59,
        ...
        function refreshDate(){
            ...
            resetInitDete();
            yearScroll.scrollTo(0, initY*-1, 100, true);
            monthScroll.scrollTo(0, initM*-1, 100, true);
            dayScroll.scrollTo(0, initD*-1, 100, true);
        }
        function refreshTime(){
            ...
            HourScroll.scrollTo(0, initH*-1, 100, true);
            MinuteScroll.scrollTo(0, initI*-1, 100, true);
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {
                var datestr =initYY.toString().substr(0,2)+$("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1)+"-"+
        ...
        function extendOptions(){
            $("#datePage").show().animate({top:180},300);
            $("#dateshadow").show();
        }
        //日期滑动
        ...
        function addTimeStyle(){
            $("#datePage").css("height","250px");
            $("#datePage").css("top","60px");
            $("#yearwrapper").css("position","absolute");
            $("#yearwrapper").css("bottom","70px");
            $("#monthwrapper").css("position","absolute");
            $("#monthwrapper").css("bottom","70px");
            $("#daywrapper").css("position","absolute");
            $("#daywrapper").css("bottom","70px");
            $("#Hourwrapper").css("position","absolute");
            $("#Hourwrapper").css("bottom","70px");
            $("#Minutewrapper").css("position","absolute");
            $("#Minutewrapper").css("bottom","70px");
        }
        //创建 --年-- 列表
